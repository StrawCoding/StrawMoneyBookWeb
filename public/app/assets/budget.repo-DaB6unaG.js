import{q as n,a as c,b as a,c as g,n as i,r as o,f as l,k as S}from"./app-google-BEAGZJNF.js";import"./vendor-sqlite-DKEMZiEb.js";import"./app-membership-ySINm0jR.js";import"./app-i18n-DtazXAJx.js";import"./vendor-CkjNk_C8.js";function A(t){return String(t??"").trim()==="daily"?"daily":"monthly"}function r(t){return String(t??"").trim()||null}function s(t){return t===!0||t===1||t==="1"}function p(t,e,d){return`${String(t??"").trim()}:${String(e??"").trim()}:${String(d??"").trim()}`}function N(t){return String(t??"").trim()==="pay_cycle"?"pay_cycle":"calendar_month"}function m(t,e){return String(t??"").trim()||String(e??"").trim()}function E(t="transactions"){const e=S(t);return`(CASE
    WHEN ABS(${t}.amount_minor) - ${e} > 0
    THEN ABS(${t}.amount_minor) - ${e}
    ELSE 0
  END)`}async function R(t,e){a(t);const d=await n(`SELECT ledger_id, id, month_key,
     ...
     LIMIT 1;`,[t,e]);return l(d)}async function v(t,e){a(t);const d=String(e??"").trim();if(!d)return null;const _=await n(`SELECT ledger_id, id, month_key,
            COALESCE(period_key, month_key) AS period_key,
            COALESCE(period_start_date, month_key || '-01') AS period_start_date,
            COALESCE(period_end_date, date(month_key || '-01', '+1 month', '-1 day')) AS period_end_date,
            COALESCE(period_mode, 'calendar_month') AS period_mode,
     ...
     LIMIT 1;`,[t,d]);return l(_)}async function x(t,e){a(t);const d=e.id??g("budget"),_=i();return await o(`INSERT INTO budgets (
      ...
      budget_save_total_enabled, budget_save_total_target_account_id, budget_save_total_source_account_id,
      created_at, updated_at
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);`,[t,d,e.monthKey,m(e.periodKey,e.monthKey),e.periodStartDate??null,e.periodEndDate??null,N(e.periodMode),e.name??null,e.includeTransfers?1:0,e.includeLoanRepayments?1:0,e.includeReimbursedExpenses?1:0,s(e.budgetSaveTotalEnabled),r(e.budgetSaveTotalTargetAccountId),r(e.budgetSaveTotalSourceAccountId),_,_]),v(t,m(e.periodKey,e.monthKey))}async function I(t){a(t);const e=await n(`SELECT b.ledger_id, b.id, b.month_key,
     ...
     ORDER BY sort_order ASC, created_at ASC;`,[t,e]);return c(d)}async function h(t,e){a(t);const d=i(),_=e.id??g("bc");return await o(`INSERT INTO budget_containers (
    ...
    ON CONFLICT(ledger_id, id) DO UPDATE SET
      ...
      updated_at = excluded.updated_at;`,[t,_,e.budgetId,e.name,Math.max(0,Math.round(Number(e.amountMinor??0))),s(e.budgetSaveEnabled),r(e.budgetSaveTargetAccountId),r(e.budgetSaveSourceAccountId),A(e.periodMode),e.sortOrder??0,d,d]),_}async function M(t,e){if(a(t),!e)return;const d=i();await o(`UPDATE budget_items
     ...
     ORDER BY bi.sort_order ASC, bi.created_at ASC;`,[t,e]);return c(d)}async function U(t,e){const d=i(),_=e.id??g("bi");return await o(`INSERT INTO budget_items (
    ...
    ON CONFLICT(ledger_id, id) DO UPDATE SET
     ...
     SET budget_save_total_enabled = ?,
         ...
         updated_at = ?
     WHERE ledger_id = ? AND id = ? AND deleted_at IS NULL;`,[s(d.enabled),r(d.targetAccountId),r(d.sourceAccountId),i(),t,e])}async function P(t,e,d){const _=await n(`SELECT category_id, SUM(${E("transactions")}) AS spent_minor
     FROM transactions
     WHERE ledger_id = ?
       AND deleted_at IS NULL
       AND type = 'expense'
     ...
     GROUP BY category_id;`,[t,e,d]);return c(_)}async function z(t,e,d){const _=await n(`SELECT category_id, substr(occurred_at, 1, 10) AS occurred_day
     FROM transactions
     WHERE ledger_id = ?
       AND deleted_at IS NULL
       AND type = 'expense'
       ...
       AND occurred_at < ?
       AND category_id IS NOT NULL
     GROUP BY category_id, substr(occurred_at, 1, 10);`,[t,e,d]);return c(_)}async function D(t,e,d,_){a(t);const u=await n(`SELECT ledger_id, id, scope_type, scope_ref_id, period_key,
     ...
     WHERE ledger_id = ?
       ...
       AND scope_ref_id = ?
       AND period_key = ?
     LIMIT 1;`,[t,e,d,_]);return l(u)}async function $(t,e={}){a(t);const d=String(e.scopeType??"").trim(),_=String(e.scopeRefId??"").trim(),u=String(e.periodKey??"").trim();if(!d||!_||!u)throw new Error("預算存款結算資料不完整");const b=i(),y=String(e.id??"").trim()||p(d,_,u);return await o(`INSERT INTO budget_save_settlements (
    ...
    ON CONFLICT(ledger_id, id) DO UPDATE SET
      ...
      updated_at = excluded.updated_at;`,[t,y,d,_,u,r(e.sourceAccountId),r(e.targetAccountId),Math.max(0,Math.round(Number(e.settledAmountMinor??0)||0)),String(e.status??"failed").trim()||"failed",r(e.transferGroupId),String(e.lastError??"").trim()||null,b,b]),D(t,d,_,u)}async function K(t,e,d,_){a(t),await o(`DELETE FROM budget_save_settlements
     WHERE ledger_id = ?
       AND scope_type = ?
       AND scope_ref_id = ?
       AND period_key = ?;`,[t,e,d,_])}export{P as computeBudgetSpentByCategory,x as createBudget,M as deleteBudgetContainer,w as deleteBudgetContainersByBudget,F as deleteBudgetItem,H as deleteBudgetItemsByBudget,K as deleteBudgetSaveSettlement,R as getBudgetByMonth,v as getBudgetByPeriod,D as getBudgetSaveSettlement,B as listBudgetContainers,k as listBudgetItems,z as listBudgetUsageDaysByCategory,I as listBudgetsWithSaveRules,W as updateBudgetSaveTotalConfig,h as upsertBudgetContainer,U as upsertBudgetItem,$ as upsertBudgetSaveSettlement};
